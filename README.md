...
Модель данных HBase реализуется по типу ключ-значение :
...
Если вы хотите самостоятельно попрактиковаться в запросах, можно использовать небольшую демо-базу. Предварительно перекиньте файл create_db на одну из запущенных Node, и выполните следующую команду:

`cat create_db | hbase shell`

или просто скопируйте содержимое файла и вставьте в hbase shell.
