...
Первичным индексом является rowkey, поддержка вторичных индексов на данный момент отсутствует, но они есть в pheonix db, которая работает поверх Hbase.
...
Для каждой из таблиц можно настроить репликацию, также есть поддержка snapshot.
...
кратенько пробежимся по основным пунктам:
